...
using EasyEditor.ReorderableList.Internal;
using System;
using UnityEditor;
using UnityEngine;
using System.Collections.Generic;

namespace EasyEditor.ReorderableList
{
	...
	public class SerializedPropertyAdaptor : IReorderableListAdaptor
	{
		...
		public virtual void DrawItem (Rect rect, int index)
		{
			...
			GUIContent content;
			//Debug.LogFormat ("propertyType = {0}", _arrayProperty.GetArrayElementAtIndex (index).propertyType);
			if (_arrayProperty.GetArrayElementAtIndex (index).propertyType == SerializedPropertyType.Generic) {
				...
				var text = _arrayProperty.GetArrayElementAtIndex (index).displayName;
				//Debug.LogFormat ("1.{0}", text);
				content = new GUIContent (text);
				#endif
			} else {
				var text = _arrayProperty.GetArrayElementAtIndex (index).displayName;
				//Debug.LogFormat ("2.{0}", text);
				content = GUIContent.none;
			}
			EditorGUI.PropertyField (rect, this [index], content, true);
		}
